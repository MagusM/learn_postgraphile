CREATE FUNCTION LEARN.HANDLE_SCORE(CARD_ID INTEGER, 
SCORE INTEGER) RETURNS LEARN.CARD AS 
	$$ DECLARE card learn.card;
	ease_factor numeric(7, 4);
	new_review_after timestamptz;
	response learn.score_response;
	BEGIN SELECT * INTO card FROM learn.card WHERE id = card_id;
	response = learn.score_response (
	    score,
	    card.spacing,
	    card.ease_factor
	...
	SET
	    ...
	    END
	WHERE
	    id = card_id RETURNING * INTO card;
	INSERT INTO
	    learn.response (
	        score,
	        review_after,
